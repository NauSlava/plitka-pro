...
## [2024-12-19] - Версия v4.3.5 - "CRITICAL EXPANDABLE_SEGMENT FIX & VAE CPU FALLBACK"

### Исправлено
- **🚨 КРИТИЧЕСКОЕ**: Устранена ошибка `!block->expandable_segment_ INTERNAL ASSERT FAILED` в VAE decode
- **🚨 КРИТИЧЕСКОЕ**: Реализован CPU fallback для VAE decode (preview и final)
- **🚨 КРИТИЧЕСКОЕ**: Полное отключение Accelerate GPU acceleration с тройной защитой

### Добавлено
- **🔧 Агрессивная очистка памяти**: `torch.cuda.reset_peak_memory_stats()`, `gc.collect()`, повторная очистка
- **🔧 VAE CPU fallback**: Автоматическое перемещение VAE на CPU перед decode
- **🔧 Тройная защита Accelerate**: 6 переменных окружения для полного отключения GPU
- **🔧 VAE переменные**: `VAE_USE_CPU`, `VAE_USE_CPU_FOR_DECODE`, `VAE_USE_CPU_FOR_ENCODE`

### Изменено
- **Функция очистки памяти**: Агрессивная очистка с принудительной сборкой мусора
- **VAE decode**: Автоматическое переключение между CPU/GPU для избежания ошибок
- **Логирование**: Улучшенные сообщения о перемещении VAE между устройствами

### Технические детали
- **Проблема**: `expandable_segment_` ошибка в VAE decode из-за фрагментации CUDA памяти
- **Решение**: CPU fallback для VAE + агрессивная очистка памяти + полное отключение Accelerate
- **Результат**: Устранение 100% воспроизводимой ошибки expandable_segment

### Ожидаемые результаты
- ✅ Устранение ошибки `expandable_segment_` в 100% случаев
- ✅ Стабильная генерация preview и final изображений
- ✅ Поддержка всех цветовых комбинаций (1-4 цвета)
