...
# 🚀 ОПТИМИЗИРОВАННОЕ подавление предупреждений - v4.3.7
import warnings
...
# Специфичные фильтры для конкретных предупреждений
warnings.filterwarnings("ignore", message=".*torch.utils._pytree.*")
# # Исправляем pytree warnings через monkey patching
...
# УБРАНО: Фильтр предупреждений Accelerate удален
# warnings.filterwarnings("ignore", message=".*accelerate.*memory.*")
warnings.filterwarnings("ignore", message=".*cache.*migration.*")

# 🚀 ОПТИМИЗАЦИЯ КЭША И СИСТЕМЫ - v4.3.7
import os
os.environ['PYTHONWARNINGS'] = 'ignore'
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
os.environ['TOKENIZERS_PARALLELISM'] = 'false'

# 🚀 НОВЫЕ: Оптимизация кэша и миграции
os.environ['HF_HUB_CACHE'] = '/tmp/huggingface_cache'  # Оптимизированный путь кэша
os.environ['HF_HUB_DISABLE_TELEMETRY'] = '1'  # Отключить телеметрию
os.environ['HF_HUB_OFFLINE'] = '0'  # Разрешить онлайн загрузку
os.environ['TRANSFORMERS_CACHE'] = '/tmp/transformers_cache'  # Кэш transformers
os.environ['DIFFUSERS_CACHE'] = '/tmp/diffusers_cache'  # Кэш diffusers
# 
# # 🚀 НОВОЕ: Предотвращение cache migration warnings
# os.environ["TRANSFORMERS_OFFLINE"] = "0"  # Разрешить онлайн загрузку
# os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "1"  # Использовать новый загрузчик
# logger.info("🔧 Cache migration warnings исправлены через переменные окружения")
...
# 🚀 ОПТИМИЗАЦИЯ ACCELERATE MEMORY - v4.3.7
# УБРАНО: Эти переменные блокировали GPU
...
# os.environ['ACCELERATE_USE_CPU_FOR_GPU'] = '1'  # Принудительно CPU для GPU операций
# os.environ['ACCELERATE_USE_CPU_FOR_GPU_IF_AVAILABLE'] = '1'  # CPU если доступен
# os.environ['ACCELERATE_USE_CPU_FOR_GPU_IF_AVAILABLE_AND_CPU_AVAILABLE'] = '1'  # Двойная проверка
# os.environ['ACCELERATE_USE_CPU_FOR_GPU_IF_AVAILABLE_AND_CPU_AVAILABLE_AND_GPU_NOT_AVAILABLE'] = '1'  # Тройная защита

# 🚀 НОВЫЕ: Дополнительные переменные для оптимизации памяти
# УБРАНО: Все переменные Accelerate удалены для устранения конфликтов
# os.environ['ACCELERATE_MEMORY_EFFICIENT_ATTENTION'] = '1'  # УБРАНО: Конфликтовало с GPU
# os.environ['ACCELERATE_GRADIENT_CHECKPOINTING'] = '1'  # УБРАНО: Конфликтовало с GPU
# УБРАНО: Эти переменные конфликтовали с GPU
# os.environ['ACCELERATE_CPU_OFFLOAD'] = '1'  # Автоматический offload на CPU при необходимости
# os.environ['ACCELERATE_SAVE_CPU_OFFLOAD'] = '1'  # Сохранять CPU offload состояние
        
# # 🚀 НОВОЕ: Исправление accelerate memory warnings
# os.environ["ACCELERATE_MAX_MEMORY"] = "0:13GB"  # Ограничиваем память до 13GB из 14.6GB
# os.environ["ACCELERATE_MEMORY_FRACTION"] = "0.9"  # Используем 90% доступной памяти
# logger.info("�� Accelerate warnings исправлены через переменные окружения")

# 🚀 ИСПРАВЛЕНО: Убраны переменные, блокирующие GPU
# os.environ['VAE_USE_CPU'] = '1'  # УБРАНО: Блокировало GPU
# os.environ['VAE_USE_CPU_FOR_DECODE'] = '1'  # УБРАНО: Блокировало GPU
# os.environ['VAE_USE_CPU_FOR_ENCODE'] = '1'  # УБРАНО: Блокировало GPU
# os.environ['ACCELERATE_NO_CUDA'] = '1'  # УБРАНО: Блокировало GPU
# os.environ['ACCELERATE_MIXED_PRECISION'] = 'no'  # УБРАНО: Блокировало GPU
...
class OptimizedPredictor(BasePredictor):
    ...
    def predict(
        ...
        # 🚀 ИСПРАВЛЕННЫЕ ПРОФИЛИ КАЧЕСТВА v4.3.15: Оптимизированные параметры для лучшего качества
        if quality == "preview":
            steps_preview, steps_final = 25, 30  # Быстрый preview
            size_preview, size_final = (512, 512), (1024, 1024)
            guidance_scale_default = 5.5  # Меньше артефактов для preview
        elif quality == "high":
            steps_preview, steps_final = 50, 80  # Высокое качество
            size_preview, size_final = (512, 512), (1024, 1024)
            guidance_scale_default = 5.0  # Оптимальное для высокого качества
        else:  # standard
            steps_preview, steps_final = 40, 60  # Стандартное качество
            size_preview, size_final = (512, 512), (1024, 1024)
            guidance_scale_default = 5.5  # Сбалансированное значение
        ...
        try:
            ...
            # 🧹 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ v4.3.10: Агрессивная очистка памяти без ограничений GPU
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                torch.cuda.synchronize()
                torch.cuda.reset_peak_memory_stats()  # Сброс статистики памяти
                gc.collect()  # Принудительная сборка мусора
                torch.cuda.empty_cache()  # Повторная очистка
                
                # 🚀 НОВОЕ: Комплексное управление памятью и устройствами
                logger.info("🔧 Комплексное управление памятью выполнено")
                
                logger.info("🧹 Комплексная очистка памяти выполнена для стабильной работы")
            
            # 🚀 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ v4.3.10: Безопасный VAE decode с совместимостью типов
            # 🚀 НОВОЕ: Оставляем ВЕСЬ pipeline на GPU для избежания конфликтов устройств
            logger.info("🔧 Final generation с полным pipeline на GPU")
        ...
        try:
            ...
            # Генерируем final с полным pipeline на GPU
            with torch.no_grad():
                final = self.pipe(**gen_params).images[0]
