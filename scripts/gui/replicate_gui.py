...
# Импортируем систему сохранения логов
try:
    from realtime_log_saver import get_log_saver
    from model_log_saver import start_model_logging, add_model_log, finish_model_logging, get_model_log_path
    LOG_SAVER_AVAILABLE = True
except ImportError:
    LOG_SAVER_AVAILABLE = False
    print("⚠️ Модуль 'realtime_log_saver' недоступен. Логи не будут сохраняться.")
...
class ReplicateWorker:
    ...
    def get_optimal_hash(self, version: str) -> str:
        ...
        # Fallback: используем VersionManager
        try:
            from version_manager import get_optimal_hash, get_current_hash
            # Сначала пробуем получить хеш для конкретной версии
            optimal_hash = get_optimal_hash(version)
            if optimal_hash and optimal_hash != "unknown":
                return optimal_hash
            # Если не получилось, используем текущий хеш
            current_hash = get_current_hash()
            if current_hash and current_hash != "unknown":
                print(f"⚠️ Используем текущий хеш: {current_hash}")
                return current_hash
            # Последний fallback
            return "bec53bf1"  # latest доступная версия
        except ImportError:
            print("⚠️ VersionManager недоступен, используем hardcoded fallback")
            return "bec53bf1"  # latest доступная версия
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            pass
        # Stop timer immediately on any stop request
        try:
    ...
    def _run_single(self, client, inputs: Dict[str, Any]) -> int:
        ...
        try:
            ...
            try:
                ...
                # Запускаем логирование модели в реальном времени
                if LOG_SAVER_AVAILABLE:
                    try:
                        start_model_logging(version_id or "latest", self._get_current_hash(), pred.id)
                        self.log(f"📁 Начато логирование модели: {get_model_log_path()}\n")
                    except Exception as e:
                        self.log(f"⚠️ Ошибка запуска логирования модели: {e}\n")
        ...
        while not self._stop_event.is_set():
            ...
            if len(logs) > last_len:
                ...
                # Сохраняем логи модели в реальном времени
                if LOG_SAVER_AVAILABLE:
                    try:
                        add_model_log(chunk)
                    except Exception as e:
                    ...
                    if stream_logs_path:
                        with open(stream_logs_path, "a", encoding="utf-8") as f:
                            ...
                            try:
                                ...
                                pass
                        # Дополнительно дублируем стартовые строки в отдельный файл для удобства
                        try:
                            if self._current_run_dir:
                                startup_live_path = os.path.join(self._current_run_dir, "startup_live.log.txt")
                                # фильтруем только строки старта/инициализации
                                important_markers = ("MODEL_START", "STARTUP_SNAPSHOT", "Инициализация модели", "🚀 ")
                                imp_lines = [ln for ln in chunk.splitlines() if any(m in ln for m in important_markers)]
                                if imp_lines:
                                    with open(startup_live_path, "a", encoding="utf-8") as sf:
                                        sf.write("\n".join(imp_lines) + "\n")
                                        try:
                                            sf.flush()
                                            os.fsync(sf.fileno())
                            ...
                            self.log(f"⚠️ Ошибка сохранения startup_live.log.txt: {type(e).__name__}: {e}\n")
                except Exception as e:
            ...
            if not started:
                if self._is_boot_error(logs) and not self._is_boot_completed(logs):
                    ...
                    self._status = "failed"
                    # Ensure timer stops on auto-cancel due to boot error
                    try:
                        self._start_time = None
                    except Exception:
                        pass
            ...
            if pred.status in ("succeeded", "failed", "canceled"):
                ...
                # Завершаем логирование модели
                if LOG_SAVER_AVAILABLE:
                    try:
                        finish_model_logging()
                        self.log(f"📊 Логирование модели завершено\n")
                    except Exception as e:
                        self.log(f"⚠️ Ошибка завершения логирования модели: {e}\n")
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        try:
            ...
            self.current_version = "v4.5.06"  # fallback  # Принудительно используем v4.5.07
        self.current_hash = "unknown"     # Хеш будет определен автоматически
        
        # Инициализируем систему сохранения логов
        self.log_saver = None
        if LOG_SAVER_AVAILABLE:
            self.log_saver = get_log_saver()
        ...
        # Создаем Frame для Text виджета с полосами прокрутки
        preset_text_frame = ttk.Frame(left)
        preset_text_frame.pack(fill=tk.X, expand=False, pady=(0,6))
        
        # Text виджет с полосами прокрутки
        self.preset_params_text = tk.Text(preset_text_frame, height=20, width=60, wrap=tk.WORD, state=tk.DISABLED, font=("Arial", 12))
        preset_v_scrollbar = ttk.Scrollbar(preset_text_frame, orient=tk.VERTICAL, command=self.preset_params_text.yview)
        preset_h_scrollbar = ttk.Scrollbar(preset_text_frame, orient=tk.HORIZONTAL, command=self.preset_params_text.xview)
        
        # Настраиваем прокрутку
        self.preset_params_text.configure(yscrollcommand=preset_v_scrollbar.set, xscrollcommand=preset_h_scrollbar.set)
        
        # Размещаем виджеты
        self.preset_params_text.grid(row=0, column=0, sticky="nsew")
        preset_v_scrollbar.grid(row=0, column=1, sticky="ns")
        preset_h_scrollbar.grid(row=1, column=0, sticky="ew")
        
        # Настраиваем веса для растягивания
        preset_text_frame.grid_rowconfigure(0, weight=1)
        preset_text_frame.grid_columnconfigure(0, weight=1)
        ...
        # Создаем Frame для Text виджета с полосами прокрутки
        log_text_frame = ttk.Frame(right)
        log_text_frame.pack(fill=tk.BOTH, expand=True)
        
        # Text виджет с полосами прокрутки
        self.text = tk.Text(log_text_frame, wrap=tk.WORD, state=tk.DISABLED, font=("Arial", 12))
        log_v_scrollbar = ttk.Scrollbar(log_text_frame, orient=tk.VERTICAL, command=self.text.yview)
        log_h_scrollbar = ttk.Scrollbar(log_text_frame, orient=tk.HORIZONTAL, command=self.text.xview)
        
        # Настраиваем прокрутку
        self.text.configure(yscrollcommand=log_v_scrollbar.set, xscrollcommand=log_h_scrollbar.set)
    ...
    def append_log(self, text: str) -> None:
        ...
        # Сохраняем лог в реальном времени
        if self.log_saver:
            self.log_saver.add_log(text)
    ...
    def _load_version_config(self) -> Optional[Dict]:
        """Загружает конфигурацию версий из scripts/config/version_config.json"""
        try:
            # Путь к файлу конфигурации версий
            base_dir = os.path.dirname(__file__)
            config_path = os.path.join(base_dir, "..", "config", "version_config.json")
            
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    return json.load(f)
            else:
                self.append_log(f"⚠️ Файл конфигурации версий не найден: {config_path}\n")
                return None
        except Exception as e:
            self.append_log(f"⚠️ Ошибка загрузки конфигурации версий: {e}\n")
            return None
    ...
    def _get_presets_path_for_version(self) -> str:
        ...
        try:
            ...
            # Загружаем конфигурацию версий из файла
            version_config = self._load_version_config()
            if not version_config:
                # Fallback к жестко заданным версиям
                version = "v4.5.01"
                self.append_log(f"⚠️ Конфигурация версий недоступна, используем {version}\n")
                filename = "test_inputs_v4.5.01_critical_fixes.json"
            else:
                # Получаем маппинг версий из конфигурации
                version_mappings = version_config.get("version_mappings", {})
                default_version = version_config.get("default_version", "v4.5.01")
                
                # Если версия неизвестна, используем дефолтную
                if not version or version == "unknown" or version not in version_mappings:
                    self.append_log(f"⚠️ Версия {version} не найдена, используем {default_version}\n")
                    version = default_version
                
                # Получаем имя файла пресетов для версии
                version_info = version_mappings.get(version, {})
                filename = version_info.get("presets_file", "test_inputs_v4.5.01_critical_fixes.json")
            base_dir = os.path.dirname(__file__)
            presets_path = os.path.join(base_dir, "..", "presets", filename)
            ...
            if os.path.exists(presets_path):
                ...
                fallback_path = os.path.join(base_dir, "..", "presets", "test_inputs_v4.5.01_critical_fixes.json")
            ...
            base_dir = os.path.dirname(__file__)
            return os.path.join(base_dir, "..", "presets", "test_inputs_v4.4.39.json")
    ...
    def update_version_info(self) -> None:
        ...
        try:
            ...
            if new_version and new_version != self.current_version:
                ...
                # Инициализируем сессию логирования
                if self.log_saver:
                    self.log_saver.start_new_session(self.current_version, self.current_hash)
    ...
    def load_window_settings(self) -> None:
        ...
        try:
            # Prefer settings next to this script (scripts/gui), fallback to project root
            local_dir = os.path.dirname(__file__)
            candidate_paths = [
                os.path.join(local_dir, "gui_settings.json"),
                os.path.join(os.getcwd(), "gui_settings.json"),
            ]
            settings_path = next((p for p in candidate_paths if os.path.exists(p)), None)
            if settings_path:
                with open(settings_path, 'r', encoding='utf-8') as f:
                    settings = json.load(f)
                
                # Устанавливаем размер и позицию окна
                geometry = settings.get('geometry', '1800x1200+100+100')
                self.root.geometry(geometry)
                
                # Устанавливаем состояние окна (обычное, максимизированное)
                state = settings.get('state', 'normal')
                if state == 'zoomed':
                    self.root.state('zoomed')
                elif state == 'iconic':
                    self.root.state('iconic')
                
                print(f"✅ Настройки окна загружены: {geometry}, состояние: {state}")
            else:
                # Настройки по умолчанию
                self.root.geometry("1800x1200+100+100")
                print("✅ Использованы настройки окна по умолчанию")
        except Exception as e:
            print(f"⚠️ Ошибка загрузки настроек окна: {e}")
            self.root.geometry("1800x1200+100+100")
    ...
    def save_window_settings(self) -> None:
        ...
        try:
            ...
            # Save near the script to keep scripts/ self-contained
            local_dir = os.path.dirname(__file__)
            settings_path = os.path.join(local_dir, "gui_settings.json")
            with open(settings_path, 'w', encoding='utf-8') as f:
                json.dump(settings, f, ensure_ascii=False, indent=2)
            
            print(f"✅ Настройки окна сохранены: {settings['geometry']}")
        except Exception as e:
            print(f"⚠️ Ошибка сохранения настроек окна: {e}")
    
    def on_window_configure(self, event) -> None:
        """Обработчик изменения размера окна"""
        # Сохраняем настройки только если это главное окно
        if event.widget == self.root:
            # Используем after для отложенного сохранения, чтобы избежать частых записей
    ...
    def on_close(self) -> None:
        ...
        # Сохраняем настройки окна перед закрытием
        self.save_window_settings()
            
        # Сохраняем финальный анализ логов
        if self.log_saver:
            self.log_saver.save_final_analysis()
